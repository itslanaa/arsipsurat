...
<head>
    ...
    <style>
        ...
        .wrapper { border: 1px solid #000; padding: 12px; }
        .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 6px; margin-bottom: 8px; }
        .title { font-weight: bold; font-size: 14pt; letter-spacing: 1px; }
        .sub { font-size: 10pt; }
        .row { display: flex; margin-bottom: 6px; }
        .label { width: 120px; font-weight: bold; }
        .value { flex: 1; }
        .box { border: 1px solid #000; padding: 6px; min-height: 70px; }
        .section-title { font-weight: bold; margin-top: 6px; margin-bottom: 4px; }
        .unit-list { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 4px 10px; }
        .check { display: flex; align-items: center; gap: 6px; }
        .square { width: 10px; height: 10px; border: 1px solid #000; text-align: center; }
        .signed { margin-top: 12px; display: flex; justify-content: space-between; }
    </style>
</head>
<body>
<div class="wrapper">
    <div class="header">
        <div class="sub">PEMERINTAH KABUPATEN BOGOR</div>
        <div class="sub">KECAMATAN CIBUNGBULANG</div>
        <div class="title">KARTU DISPOSISI</div>
    </div>

    <div class="row"><div class="label">Asal Surat</div><div class="value">: <?= htmlspecialchars($asal); ?></div></div>
    <div class="row"><div class="label">Tanggal Surat</div><div class="value">: <?= $tglSurat; ?></div></div>
    <div class="row"><div class="label">Tanggal Terima</div><div class="value">: <?= $tglTerima; ?></div></div>
    <div class="row"><div class="label">Nomor Agenda</div><div class="value">: <?= htmlspecialchars($agenda); ?></div></div>
    <div class="row"><div class="label">Perihal</div><div class="value">: <?= htmlspecialchars($perihal); ?></div></div>
    <div class="row"><div class="label">Klasifikasi</div><div class="value">: <?= htmlspecialchars($kode); ?> <?= $namaKat ? '(' . htmlspecialchars($namaKat) . ')' : ''; ?></div></div>
